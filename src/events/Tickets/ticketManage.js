...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!data) return interaction.reply({embeds: [embed.setColor('Red').setDescription(client.config.ticketError)], ephemeral: true}).catch(error => {return});
        ...
        if(!findMembers) {
            ...
            interaction.channel.send({embeds: [embed.setColor('Green').setDescription('<@' + interaction.values[0] + '>' + ' ' + client.config.ticketMemberAdd)]}).catch(error => {return});
            data.save();
        }else {
            data.MembersID.remove(interaction.values[0]);
            channel.permissionOverwrites.delete(interaction.values[0]).catch(error => {return});
            interaction.channel.send({embeds: [embed.setColor('Green').setDescription('<@' + interaction.values[0] + '>' + ' ' + client.config.ticketMemberRemove)]}).catch(error => {return});
            data.save();
        }
    }
}