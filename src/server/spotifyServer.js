...
async function startServer() {
    try {
        const server = app.listen(PORT, () => {
            console.log(`${color.green}[${getTimestamp()}]${color.reset} [SPOTIFY_SERVER] Local server running on port:${color.torquise} http://localhost:${PORT}`);
        });
        ...
        console.log(`${color.green}[${getTimestamp()}]${color.reset} [SPOTIFY_SERVER] Public URL:${color.torquise} ${url} ${color.reset}`);

        server.on('error', (error) => {
            if (error.code === 'EADDRINUSE') {
                console.error(`${color.red}[${getTimestamp()}]${color.reset} [SPOTIFY_SERVER] Port ${textEffects.bold}${PORT}0 is already in use.`);
            } else {
