...
const disabled = require("../../schemas/automod");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) return await interaction.reply({ content: `You don't have perms to manage the anti ghost ping system`, flags: MessageFlags.Ephemeral});
        ...
        switch (sub) {
            ...
            if (Data) return await interaction.reply({ content: `You already have the anti ghost ping system setup`, flags: MessageFlags.Ephemeral});
            else {
            ...
            if (!Data) return await interaction.reply({ content: `There is no anti ghost ping system setup here`, flags: MessageFlags.Ephemeral});
            else {
            ...
            if (!data) return await interaction.reply({ content: `This member doesn't have any ghost pings yet`, flags: MessageFlags.Ephemeral});
            else {
