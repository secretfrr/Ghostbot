...
module.exports = {
    ...
    async execute(interaction, client) {
      ...
      try {
        ...
        await interaction.reply({ content: `<:alert1:1266583171147759728> Failed to disable the slowmode.`, ephemeral: true });
      }
      ...
      try {
        ...
        if (slowmode === 0) {
          await interaction.reply({ content: `<:alert1:1266583171147759728> Slowmode is not enabled in ${channel.name}.`, ephemeral: true });
        } else {
          ...
          .setTimestamp()
          .setFooter({ text: `discord.gg/ghostt` });

          await interaction.reply({ embeds: [slowmodeCheckEmbed], ephemeral: true });
        }
      } catch (error) {
        client.logs.error("Failed to check slowmode:", error);
        await interaction.reply({ content: "<:alert1:1266583171147759728> Failed to check slowmode status in this channel. This could be due to lack of permissions or just a one off. Please try again.", ephemeral: true });
      }
    }
  },
};
