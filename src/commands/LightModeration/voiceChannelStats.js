    ...
    async execute(interaction, client, err) {
        ...
        switch (sub) {
            ...
            if (!voiceData) {
                ...
                voiceTotalChannel.setName(`Members: ${interaction.guild.memberCount}`).catch(err);
            ...
            if (!botData) {
                ...
                botGuildChannel.setName(`烙 Bots: ${botCount}`).catch(err);
                ...
                .setTimestamp()
                .setTitle(` Ghost`)
                .addFields({ name: `> Your channel (${botGuildChannel}) has been set \n> up to be your total bots \n> voice channel! It will now display your \n> total bots accordingly.`})
                .setFooter({ text: `🔊 Total Channel Set`})
